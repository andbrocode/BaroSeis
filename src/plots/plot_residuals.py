"""
Module for plotting residuals between observed and predicted data.
"""
...
def plot_residuals(st: Stream, config: Dict[str, Any], time_unit: str = "minutes", 
    ...
    for i, comp in enumerate(components):
        ...
        try:
            ...
            if tr_pred is not None:
                ...
                coef_text = ""
                if 'pressure_ratios' in config and 'hilbert_ratios' in config:
                    if comp in config['pressure_ratios'] and comp in config['hilbert_ratios']:
                        p_coef = config['pressure_ratios'][comp] *1e9 *1e2  # Convert to nano units and hPa
                        h_coef = config['hilbert_ratios'][comp] *1e9 *1e2  # Convert to nano units and hPa
